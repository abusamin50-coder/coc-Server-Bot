...
3️⃣  Auto-uses:
   ✅ server/render.yaml (deployment config)
...
MAIN FILES
  ...
  server/wsgi.py             → Render entry point
...
CONFIG
  ...
  server/Procfile            → Render startup
  server/render.yaml         → Render manifest
...
Deploy: Push to GitHub → Connect Render service
""")
